"""
main.py — MunimAI FastAPI backend (spec Sections 9 & 10).
...
import matcher as M
import ledger as L
# Real date, not a constant frozen at import; MUNIM_TODAY pins it for the demo.
...
app = FastAPI(title="Munim.ai — Awaaz se hisaab")
...
_CURRENT: ContextVar = ContextVar("munim_repo", default=None)
_CURRENT_USER: ContextVar = ContextVar("munim_user", default=None)
...
async def _resolve_user(request, call_next):
    ...
    if not token:
        token = request.cookies.get("munim_session") or ""
    user = None
    try:
...
def _auth_response(user: dict):
    ...
    out.set_cookie("munim_session", token, max_age=auth.SESSION_DAYS * 86400,
...
def auth_logout(request: Request):
    token = (request.headers.get("authorization") or "").removeprefix("Bearer ").strip()
    auth.revoke_session(token or request.cookies.get("munim_session") or "")
    out = JSONResponse({"ok": True})
    out.delete_cookie("munim_session")
    return out
...
def send_reminders(payload: dict = Body(default={}),
    ...
    expected = (os.environ.get("MUNIM_CRON_TOKEN") or "").strip()
...
def cron_reminders(request: Request):
    ...
    `Authorization: Bearer $CRON_SECRET`; MUNIM_CRON_TOKEN is accepted too so
    the job can be triggered by hand.
    """
    import notify
    expected = (os.environ.get("CRON_SECRET")
                or os.environ.get("MUNIM_CRON_TOKEN") or "").strip()
    if not expected:
...
def converse(payload: dict = Body(...)):
    ...
    Munim should SAY (text + Bulbul audio), whether to keep listening, the
