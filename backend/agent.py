...
Munim's own dialogue engine (conversation.py) is a hand-written controller:
...
cookie, so identity arrives one of two ways:
  ...
  * ``shop_key`` — an unguessable per-shop key derived from MUNIM_SECRET,
...
def shop_key(user_id: str) -> str:
    ...
    and tied to MUNIM_SECRET so rotating that invalidates every issued key.
    """
    secret = os.environ.get("MUNIM_SECRET") or ""
    if not secret:
        raise AgentError("MUNIM_SECRET is not configured.")
