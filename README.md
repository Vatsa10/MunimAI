...
Munim.ai is a voice-first **warehouse and shop operating system** for Indian
...
> Most inventory systems assume perfect data entry. Munim.ai is designed for
...
- **Multilingual by design.** Munim supports all languages provided by Sarvam
...
- **Self-learning for every shop.** Confirmed product matches and corrections
  teach Munim that shop's aliases, preferred units, and attribute patterns, so
...
| --- | --- |
| Voice Entry | Talk to Munim, record work, ask questions, and review the latest bill or summary |
...
MUNIM_SECRET=replace-with-a-long-random-value
MUNIM_PUBLIC_URL=http://127.0.0.1:8000
...
For demo environments, set `MUNIM_TEST_RECIPIENT`. Every outbound WhatsApp
...
Remove `MUNIM_TEST_RECIPIENT` only when the deployment is intentionally ready
...
media without a Munim.ai login. The random document token and expiry protect
the file.
...
sed -n '/^<script>$/,/^<\/script>$/p' frontend/index.html \
  | sed '1d;$d' > /tmp/munim-inline.js
node --check /tmp/munim-inline.js
```
...
5. keep `MUNIM_TEST_RECIPIENT` set during demos;
6. set `MUNIM_PUBLIC_URL` to the production HTTPS origin;
